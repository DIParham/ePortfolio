...
struct Bid {
    ...
    string title; // unique identifier for second bst
    string fund;
    double amount;
    Bid() {
        amount = 0.0;
...
private:
    ...
    void insertID(Bid bid, Node* newNode);
    void preOrder(Node* node);
    Node* removeNode(Node* node, string bidId);

    //uses bid title as key
...
void BinarySearchTree::InOrder() {
    //sorts bids based on bid id
    this->inOrder(this->root);
}
/**
* Transverse tree pre order by id
*/
void BinarySearchTree::PreOrder() {
    this->preOrder(this->root);
}
/**
*Transverse tree in title order
*/
void BinarySearchTree::TitleOrder() {
    //sorts bids in alphabetical order 
    this->titleOrder(this->titleRoot);
}
/**
* Transverse tree in Post Order by Title
*/
